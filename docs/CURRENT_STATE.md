...
- **v1.4.2~v1.4.3 심사 대비 방어 (2026-07-20)** — 외부 심사위원 관점 리뷰에서 나온 실사용 결함 2건 조치: ① **Vision 버튼이 항상 실패하던 문제** — 분류기 모델(tflite)이 배포본에 없어 [AI가 알아봐요]를 누르면 100% 폴백 화면이 떴다. 카드 버튼이 이미 쓰던 자산 존재 확인 패턴을 공용 함수(`gateButtonOnAsset`)로 뽑아 Vision에도 적용 — 모델이 없으면 "준비 중 🚧" 비활성, 배치하면 코드 변경 없이 자동 활성화, `?visionMock=`은 항상 우회. E2E S0가 이 상태를 검증(음성통제 확인). ② **fbx에 구워진 제작 PC 절대경로** — `raoni.fbx`·`raona.fbx`의 `raoni_texture-01.jpg` 참조가 프로덕션에서 매번 404를 냈다(화면은 머티리얼 규칙이 대체해 정상이라 눈에 안 띄었음). ⚠️ v1.4.3의 1차 수정(백슬래시·드라이브 문자 검사)은 무효였음이 코드 리뷰에서 판명 — FBXLoader가 URL 수정자에 넘기기 전에 경로를 파일명으로 정규화해 그 패턴이 절대 도달하지 않았고, preview의 SPA 폴백(404→index.html 200)이 검증까지 가렸다. v1.4.5에서 배포 파일 화이트리스트 + 데이터 URI 센티널 방식으로 재수정: 목록 밖 요청은 네트워크 대신 센티널로 가고, 머티리얼 규칙은 센티널을 '깨진 맵'으로 간주해 외형(라오나 순색 피부 포함)을 그대로 유지한다. 검증도 상태 코드가 아니라 '요청 발생 0건'으로 확인. 파비콘도 추가해 루트 `/favicon.ico` 404를 없애고 하네스의 예외 분기를 삭제 — 이제 E2E "콘솔 에러 0건"에 파비콘 예외가 없다. 테스트 127→132개. README는 체험 모드 5종·E2E 11종·유닛 수치·스크린샷까지 현행화.

- **v1.4.4 오프라인 큐 보강 (2026-07-20)** — 행사 리스크 점검 중 발견: 큐 재전송 트리거가 "앱 부팅 시 1회"뿐이라, 부스 태블릿처럼 리로드 없이 오래 켜두는 기기에서는 네트워크가 복구돼도 다음 리로드까지 응답 수집이 밀렸다(유실은 아니고 지연). `online` 이벤트에도 flush를 걸어 해결. E2E S11(오프라인 다건 누적 → 리로드 유지 → 네트워크 복구 시 리로드 없이 자동 비움 → 리로드 후 되살아나지 않음) 신설 — 리스너를 빼면 S11이 실패하는 것을 음성통제로 확인. `npm test`의 `--passWithNoTests`도 제거(글롭이 깨져 테스트 0개가 돼도 CI가 조용히 통과하던 구멍). E2E 11종→12종.

## 아직 안 된 것

[NEXT_STEP.md](NEXT_STEP.md) 참고. 요약: 구글 폼 동의 문항 entry 연결, 대시보드 시트 연동(csvUrl), 실기기 테스트, 시연 영상·신청서 제출, 별점 1~2점 "sad" 리액션 유지 여부 결정.

- **실기기 피드백 5건 수정 (2026-07-21)** — ① 말풍선·레이어 클릭 차단 해소(elementFromPoint 실측) ② 카드 소환 후 캐릭터 소실 — MindAR 픽셀 단위·near 클리핑·attach NaN 3중 원인 근본 수정 + 놓침 시 카메라 홀드 ③ Quick Look 앵커 규격화(이후 기능 자체 제거) ④ 자이언트 체감 재보정 ⑤ 레트로 툰 룩(인버티드 헐 외곽선 + 5단 램프 + 스튜디오 조명), 눈·초저휘도 그룹은 외곽선 제외(쩜눈 복원).
- **쇼케이스 전환 + 카드 단일화 (2026-07-21, v1.5)** — [만나러 가기]를 카메라·자이로·권한 없이 뜨는 쇼케이스(고정 그라데이션 배경)로 전환, 크기 칩 접기(?size= URL 전용). Quick Look·WebXR 제거 — AR 표면은 카드 소환 하나로 단일화. 바닥용 대형 카드 인쇄물 3종(SVG, `docs/print/floor-card-*.svg`) 추가.
- **리깅 결함 2건 근본 수정 (2026-07-21)** — 팔 흔들 때 얼굴 늘어남(웨이트 누출 본 자동 제외, 이름 기반 매칭) + 외곽선 팔 어긋남(외곽선 헐을 부모 스켈레톤에 바인딩된 스킨드 메시로 전환, 스키닝 셰이더 청크). 유닛 131개·E2E 12종 그린.
- **시연 영상 풀자동 제작 (2026-07-22)** — 실촬영 0초로 12컷 63.5초 두 버전(자막+BGM / +TTS 내레이션) 완성. E2E 하네스 재사용(CDP 프레임 수집 — puppeteer ScreenRecorder 시간축 왜곡 실측 후 우회), 카드 소환 컷은 y4m 합성 카메라로 실제 MindAR 인식 장면. 도구·산출물은 리포 밖(`../video/`). 유튜브 한정공개 업로드 + 신청 폼 첨부 완료. 내레이션 겹침 수정판(final-narration-fixed) 별도 생성.
- **신청서 문안 확정 (2026-07-22)** — 실제 폼(PDF)과 문항 대조 완료, 폼 입력까지 완료(최종 제출 버튼만 남음). "잠자던 산출물 부활" 서사는 다른 팀 활용 가능성이 있어 "3D 원본의 쓰임을 AR로 확장" 프레임으로 전면 교체(문안·영상·설계서 동기화). 팀원 표기는 대표 구분 없이 가나다순.

**최종 갱신(2차)**: 2026-07-22 — 리포가 `~/Desktop/20231127_산출물/raon-friends-ar`로 이동됨(산출물 아카이브 통합).

- **바닥 포스터 fiducial 인식 (2026-07-22, ADR 0002)** — 대형 포스터 실기기 인식 불량·기울어짐의 근본 원인(사선 시점에서 NFT 특징이 같은 템플릿의 다른 카드 타겟에 교차 매칭 — 변수 분리 y4m 계측으로 확정)을 ArUco 모서리 마커 4점으로 해결. ID가 캐릭터+모서리를 인코딩해 오인식 원천 차단, 포즈는 전 마커 꼭짓점 호모그래피 분해(마커별 POSIT의 평면 2중 해 모호성 회피, js-aruco2 svd 버그로 야코비 고유분해 자체 구현). 진입 UI 불변([카드로 소환하기] 내 자동 전환 + NFT veto). 계측: 사선 45°+이음새 조건에서 진입 2.1s·화자 정합·기립 upDot 0.968·표본 폭 0.004("딱 붙음"). 인쇄물 3종에 마커 주입(`scripts/inject-poster-markers.mjs`, 멱등). E2E S12(성공)·S13(음성통제) 추가 — 14종. 부수 수정: e2e 하네스 `URL.pathname` → `fileURLToPath`(한글 경로 이전으로 노출된 잠복 버그). **배포 완료**(4ebd18e) 후 실기기 1차 확인(A4 마커판): 소환·기립·부착 정상 — 크기 피드백 반영해 캐릭터 키를 포스터 폭 1.3배 → **2.2배**(포스터를 덮는 크기)로 조정·배포(66adb20).
