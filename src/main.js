...
// (텍스트 내용은 기존 guideScript 그대로, speaker만 교체 — 마커 모드의 "단독 진행"과 동일한 화자 고정 방식)
const guideScript = (soloCharParam && CONFIG.characters[soloCharParam])
  ? CONFIG.guideScript.map((line) => ({ ...line, speaker: soloCharParam }))
  : CONFIG.guideScript;

const flow = createFlow(guideScript);
let overlay = null;
