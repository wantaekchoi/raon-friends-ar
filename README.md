...
> 만나기(쇼케이스) 모드에서는 라옹 → 라오니 → 라오나 순으로 배턴터치하며 등장합니다. 카드 마커 모드는 비춘 카드의 캐릭터가 그대로 진행을 맡아요. 세계관 출처: [raon.com 캐릭터 페이지](https://www.raon.com/ko/intro/company/character) · [회사 소개](https://www.raon.com/ko/about/company)

---

## 🎭 체험 모드

같은 안내·설문 흐름을 두 가지 방식(+ 준비 중인 Vision AI)으로 체험할 수 있어요 — 빠르고 실패 없는 쇼케이스, 그리고 실제 공간에 붙는 카드 AR.

| 모드 | 지원 범위 | 사용법 |
|---|---|---|
| **✨ 만나기(쇼케이스)** | 전 기기 | [✨ 라온 프렌즈 만나러 가기]를 누르면 카메라·권한 없이 예쁜 배경 위에서 캐릭터가 바로 등장해 안내를 진행해요 — 가장 빠르고 실패 없는 기본 경로예요. 실제 공간(바닥·카드)에 붙는 AR을 원하면 아래 카드 마커 모드를 사용하세요. |
| **🃏 카드 마커 모드** | MindAR 지원 브라우저 | [카드로 소환하기]를 누른 뒤 카드(또는 화면에 띄운 카드 이미지)를 비추면, 그 카드 자리에 정확히 캐릭터가 올라타요(6DoF 부착). 소환 후에도 카드가 보이는 동안은 카드에 딱 붙어 있고, 카드를 놓치면 화면에 남았다가 다시 비추면 그 자리로 착 돌아와요. **바닥용 대형 포스터**(`docs/print/floor-card-*.svg`)는 모서리의 ArUco 마커 4점으로 자동 인식돼요 — 크게 확대 인쇄해도, 서서 비스듬히 비춰도 캐릭터가 포스터 위에 수직으로 딱 서요(fiducial 6DoF — 그림 자체를 지문처럼 인식하는 NFT(자연 특징점 추적) 방식이 사선에서 흔들리는 문제의 해법). |
...
- 🖨️ **인쇄용 PDF(손 카드)**: [`docs/cards-print.pdf`](docs/cards-print.pdf) — 카드 3장을 A4 한 장에 원본 해상도로 배치. 코팅/무광 용지 인쇄를 추천해요(조명 반사가 인식을 방해할 수 있어요).
- 🏟️ **바닥용 대형 카드(캐릭터별 SVG, A4 세로 공통 템플릿)**: [`라옹`](docs/print/floor-card-raong.svg) · [`라오니`](docs/print/floor-card-raoni.svg) · [`라오나`](docs/print/floor-card-raona.svg) — 브라우저로 열어 인쇄하세요. **캐릭터 키는 카드 폭에 비례**하므로(약 1.2×카드 폭) 부스 바닥에 크게 깔수록 캐릭터가 크게 서서 손님을 맞아요 — SVG라 A3 이상으로 확대해도 레이아웃이 그대로예요.
...
- **🎭 체험 모드** — 만나기(쇼케이스: 카메라·권한 없이 즉시 등장) · 카드 마커 AR(MindAR 6DoF 부착) · Vision AI 인식(준비 중, 아래 참고). 위 [체험 모드](#체험-모드) 참고.
...
| `?size=` | `life` \| `giant` | 캐릭터 크기(매직 미러 부스 세팅용 URL 전용 옵션). 기본 1.2m, `life` 1.8m, `giant` 3.2m. |
...
npm test          # Vitest 실행
npm run e2e       # E2E 시나리오 14종 (빌드 → preview 기동 → headless Chrome, fake 카메라)
```

<details>
<summary>E2E 시나리오 14종 (S0~S13) — 각각 무엇을 검증하는지</summary>
...
| S12 | **포스터(fiducial) 소환** | 마커 포함 바닥 포스터를 "서서 비스듬히(45°) 보는" 합성 카메라(y4m, 진짜 핀홀 원근)로 비춰 — NFT가 교차 오인식하던 바로 그 조건에서 포스터 모드 진입·화자 정체성(라옹 포스터→라옹)·기립 안정(`?posterDebug` 법선 표본)·화면 내 위치까지 검증 |
| S13 | **포스터 음성통제** | 같은 조건에서 ArUco 마커만 제거한 구판 포스터 → 포스터 모드가 절대 켜지지 않아야 함(오검출 방지) |
...
│   │   └── scenarios/           # S0~S13 14종 — 위 [로컬 실행](#-로컬-실행) 절 표 참고
